                ...
                <div class="image">
                    <div class="dropdown" style="position: absolute; top: 15px; right: 10px; z-index: 999;" id="dropdown-{{ $organik->id }}">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('pestisida.edit', $organik->id) }}">Edit</a>
                                <a class="dropdown-item" href="#" onclick="confirmDelete('{{ $organik->id }}')">Delete</a>
                                
                                <form id="delete-form-{{ $organik->id }}" action="{{ route('pestisida.delete', $organik->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
                    ...
                    @if(strtolower(pathinfo($organik->file, PATHINFO_EXTENSION)) === 'mp4')
                        ...
                        <video width="100%" height="auto" controls poster="{{ asset('storage/' . $organik->cover) }}">
                            <source src="{{ asset('storage/' . $organik->file) }}" type="video/mp4">
                            Your browser does not support the video tag.
                        </video>
                    @else
                        <!-- Jika bukan MP4, tampilkan gambar -->
                        <img src="{{ asset('storage/' . $organik->cover) }}" />
                    @endif
                </div>
                <div class="card-inner">
                    ...
                    <div class="content">
                        @if(strtolower(pathinfo($organik->file, PATHINFO_EXTENSION)) === 'mp4')
                            ...
                            <p><a href="{{ asset('storage/' . $organik->file) }}" >download</a></p>
                        @else
                            <!-- Jika bukan MP4, tampilkan link download -->
                            <p><a href="{{ asset('storage/' . $organik->file) }}" download>download</a></p>
                        @endif
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        @foreach($organiks as $organik)
            var dropdown{{ $organik->id }} = document.getElementById('dropdown-{{ $organik->id }}');
            
            dropdown{{ $organik->id }}.addEventListener('click', function () {
                var menu = dropdown{{ $organik->id }}.querySelector('.dropdown-menu');
